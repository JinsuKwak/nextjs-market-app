...
const ImageUpload = ({ onChange, value }: ImageUploadProps) => {
  ...
  const uplaodPreset = process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET;

  return (
    <CldUploadWidget onUpload={handleUpload} uploadPreset={uplaodPreset} options={{ maxFiles: 1 }}>
      {({ open }) => {
        return (
          <div
            onClick={() => open?.()}
          ...
          >
            <TbPhotoPlus size={50} className="text-neutral-400" />
            {value && (
